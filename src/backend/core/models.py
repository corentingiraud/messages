...
class Channel(BaseModel):
    ...
    # uniqueness hashes the bare key (push: ``sha256(token)``); one wanting
    # per-user/per-mailbox uniqueness folds that id in (``sha256(f"{user_id}:{key}")``);
    # and every type should namespace its input (e.g. a type prefix) so two
    # types can never collide on the same raw value.
