...
def send_fcm(items: list[tuple[models.Channel, dict]], collapse_key: str) -> PushResult:
    ...
    """
    if not items or not _fcm_configured():
        return PushResult()
    ...
    # Process-global client, reused across notification tasks (keep-alive to the
    # single FCM host avoids a TLS handshake per push).
    client = _fcm_client()
