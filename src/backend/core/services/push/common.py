...
# token) — an indexed, globally-unique column, so we never put the token itself
# in a queryable column.
# ---------------------------------------------------------------------------
...
def _channel_token(channel: models.Channel) -> str | None:
    return (channel.encrypted_settings or {}).get("token")


def _channel_keys(channel: models.Channel) -> dict | None:
    return (channel.encrypted_settings or {}).get("keys")
...
def register_push_device(
    *,
    ...
    registers it. ``Channel.lookup_hash`` (sha256 of the token) is globally
    unique. Re-registering by the *same* user updates that user's row in place
    ...
    with transaction.atomic():
        # Drop any row for this token owned by a *different* user, so the upsert
        # below creates a fresh channel for the caller rather than inheriting the
        # other user's row. The DB unique index on lookup_hash keeps this
        # race-safe: a concurrent first-time insert that loses surfaces as
        # IntegrityError, which update_or_create catches and retries as an update.
        models.Channel.objects.filter(
            type=ChannelTypes.PUSH, lookup_hash=token_hash
        ).exclude(user=user).delete()

        # ``defaults`` (update path) omits ``name`` so a same-user refresh keeps
        # the device's existing label; ``create_defaults`` sets it on first
        # registration (``name`` is a required field, so it must be present when
        # the row is first saved).
        ...
        try:
            ...
            # Lost a concurrent first-time registration of the same token:
            # BaseModel.save runs full_clean, whose own uniqueness check raises
            # ValidationError (before any INSERT, so the transaction is still
            # usable) rather than the IntegrityError update_or_create retries on.
            # The winning row is now committed, so a single retry resolves to the
            # update path.
            channel, created = models.Channel.objects.update_or_create(
                lookup_hash=token_hash,
                defaults=common,
...
def _apns_client() -> httpx.Client:
    ...
    """
    global _APNS_CLIENT  # noqa: PLW0603
...
def _fcm_client() -> httpx.Client:
    ...
    global _FCM_CLIENT  # noqa: PLW0603
...
def _close_push_clients(**_kwargs):
    ...
    global _APNS_CLIENT, _FCM_CLIENT  # noqa: PLW0603
